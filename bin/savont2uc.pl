...
#
# This is an optional standalone converter. The LotuS3 ONT pipeline uses
# backmapping for abundances and does not invoke this script.
#
# WHY THIS EXISTS
...
die "--idmode must be firsttoken or full\n" unless $idmode eq "firsttoken" || $idmode eq "full";
...
my $otuIdx = 0;
{
    ...
    while (my $line = <$fh>) {
        ...
        if ($line =~ /^>/) {
            ...
            if ($line =~ /debug_id:(\d+)/) {
                $curDbg = $1;
                die "Duplicate ASV debug_id:$curDbg in $asvFile\n" if exists $dbg2otu{$curDbg};
                $otuIdx++;
